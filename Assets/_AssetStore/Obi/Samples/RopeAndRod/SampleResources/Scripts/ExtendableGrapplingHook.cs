...
public class ExtendableGrapplingHook : MonoBehaviour
{
    ...
    private IEnumerator AttachHook()
    {
        ...
        while (true)
        {
            ...
            if ((rope.sourceBlueprint.particleCount - 1) * rope.ropeBlueprint.interParticleDistance < distance)
            {
                direction = Vector3.ClampMagnitude(direction, (rope.sourceBlueprint.particleCount - 1) * rope.ropeBlueprint.interParticleDistance);
                hookAttachment.point = origin + direction;
            }
